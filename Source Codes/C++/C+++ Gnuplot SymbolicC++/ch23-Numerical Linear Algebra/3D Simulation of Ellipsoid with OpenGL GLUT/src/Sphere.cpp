...
void Sphere::printSelf() const
{
	std::cout << "======== Sphere & Ellipsoid ========\n"
              ...
              << "        Ellipsoid parameter a: " << a << "\n"
              << "        Ellipsoid parameter b: " << b << "\n"
              << "        Ellipsoid parameter c: " << c << "\n"
...
void Sphere::buildVerticesSmooth()
{
	...
	for(int i = 0; i <= stackCount; ++i)
	{
		...
		// z = h * sinf(stackAngle) * cosf(stackAngle) -> Doughnut
		// z = h * cosf(stackAngle) -> Cone		
		z = c * radius * sinf(stackAngle);              // r * sin(u)
		...
		for(int j = 0; j <= sectorCount; ++j)
		{
			...
			// vertex position
			x = a * xy * cosf(sectorAngle);             // r * cos(u) * cos(v)
			y = b * xy * sinf(sectorAngle);             // r * cos(u) * sin(v)
			addVertex(x, y, z);
