...
int main() {
	...
	float w2 = 2;
	float w3 = 3;
	float innerproduct_u2v1 = w1*u1[0]*v1[0] + w2*u2[1]*v1[1] + w3*u2[2]*v1[2]; 
	float norm_v1 = sqrt(w1*v1[0]*v1[0] + w2*v1[1]*v1[1] + w3*v1[2]*v1[2]);

	u2projW1 = (innerproduct_u2v1 / (pow(norm_v1,2)))*v1;	
	v2 = u2 - u2projW1;
	float norm_v2 = sqrt(w1*v2[0]*v2[0] + w2*v2[1]*v2[1] + w3*v2[2]*v2[2]);

	float innerproduct_u3v1 = w1*u3[0]*v1[0] + w2*u3[1]*v1[1] + w3*u3[2]*v1[2]; 
	float innerproduct_u3v2 = w1*u3[0]*v2[0] + w2*u3[1]*v2[1] + w3*u3[2]*v2[2]; 

	u3projW2 = (innerproduct_u3v1 / (pow(norm_v1,2)))*v1 + (innerproduct_u3v2 / (pow(norm_v2,2)))*v2;	
	v3 = u3 - u3projW2;
	float norm_v3 = sqrt(w1*v3[0]*v3[0] + w2*v3[1]*v3[1] + w3*v3[2]*v3[2]);

	cout <<"\nv1 :\n" << v1 <<endl;
	cout <<"v2 :\n" << v2 <<endl;
	cout <<"v3 :\n" << v3 <<endl;
	cout <<"\n|| v1 || :"  << norm_v1 <<endl;
	cout <<"|| v2 || :"  << norm_v2 <<endl;
	cout <<"|| v3 || :"  << norm_v3 <<endl;

	float innerproduct_v1v2 = w1*v1[0]*v2[0] + w2*v1[1]*v2[1] + w3*v1[2]*v2[2]; 
	float innerproduct_v1v3 = w1*v1[0]*v3[0] + w2*v1[1]*v3[1] + w3*v1[2]*v3[2]; 
	float innerproduct_v2v3 = w1*v2[0]*v3[0] + w2*v2[1]*v3[1] + w3*v2[2]*v3[2]; 

	cout <<"\n<v1,v2> :" << innerproduct_v1v2 <<endl;
	cout <<"<v2,v3> :" << innerproduct_v2v3 <<endl;
	cout <<"<v1,v3> :" << innerproduct_v1v3 <<endl;
	...
	float norm_q1 = sqrt(w1*q1[0]*q1[0] + w2*q1[1]*q1[1] + w3*q1[2]*q1[2]);
	float norm_q2 = sqrt(w1*q2[0]*q2[0] + w2*q2[1]*q2[1] + w3*q2[2]*q2[2]);
	float norm_q3 = sqrt(w1*q3[0]*q3[0] + w2*q3[1]*q3[1] + w3*q3[2]*q3[2]);

	cout <<"\n|| q1 || :"  << norm_q1 <<endl;
	cout <<"|| q2 || :"  << norm_q2 <<endl;
	cout <<"|| q3 || :"  << norm_q3 <<endl;
	...
	*/
	...
	/*	
	gp << "splot '++' using 1:2:(0) with lines lw 2 lc rgb 'gray' title 'W = (x,y,0)', \
		'-' with vectors lw 3 lc rgb 'salmon' title 'q_{1}',\
		'-' with vectors lw 3 lc rgb 'magenta' title 'q_{2}', '-' with vectors lw 3 lc rgb 'aquamarine' title 'q_{3}'\n";
