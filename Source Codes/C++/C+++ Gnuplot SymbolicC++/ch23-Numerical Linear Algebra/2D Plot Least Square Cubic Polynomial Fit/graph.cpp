// g++ graph.cpp -o graph ../common/shaders_utils.o -lm -lglut -lGLEW -lGL -larmadillo
...
float scale_x = 0.05; // first look: stretching out the x-axis when value is 0 < scale_x < 1
float scale_y = 0.0002; // first look: stretching out the y-axis when value is 0 < scale_y < 1
int mode = 0;
...
int init_resources() {
	...
	if (attribute_coord2d == -1 || uniform_offset_y == -1 || uniform_offset_x == -1 || uniform_scale_y == -10 || uniform_scale_x == -1 || uniform_sprite == -1 || uniform_mytexture == -1)
		return 0;
...
void display() {
	...
	case 3:
		...
		point graphcubicfit[2000]; 
		for (int i = 0; i < 2000; i++) 
		{
			float x = (i -1000) / 10.0;
			graphcubicfit[i].x = x;
			graphcubicfit[i].y = V[0] + V[1]*x + V[2]*x*x + V[3]*x*x*x;		
		}	
	
		glBufferData(GL_ARRAY_BUFFER, sizeof graphcubicfit, graphcubicfit, GL_STATIC_DRAW);
...
void special(int key, int x, int y) {
	...
	case GLUT_KEY_F4:
		mode = 3;
		printf("Now drawing the cubic polynomial fit.\n");
		break;
	case GLUT_KEY_LEFT:
		offset_x += 2;
		break;
	...
	case GLUT_KEY_HOME:
		offset_x = 3.0; 
		offset_y = 100.0;
		scale_x = 0.05; // stretching the x-axis
		scale_y = 0.0002; // stretching the y-axis
		break;
	}
...
int main(int argc, char *argv[]) {
	...
	glutCreateWindow("Least Squares Cubic Polynomial Fit");
	...
	cout << "Least Squares Cubic Polynomial Fit :" << endl;
	cout << ("y = ");
	printf("%.3f + ", V[0]);
	printf("%.3f x + ", V[1]);
	printf("%.3f x^2 + ", V[2]);
	...
	printf("Press F4 to draw the least squares cubic polynomial fit.\n");
