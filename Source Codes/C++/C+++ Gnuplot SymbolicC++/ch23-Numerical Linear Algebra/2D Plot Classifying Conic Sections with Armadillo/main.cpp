...
int main(int argc, char** argv)
{
	...
	mat I(2,2,fill::eye);
	// A conic with equation ax^2 + bxy + cy^2 - k = 0	
	float a = 5;
	float b = -4;
	float c = 8;
	float k = 36;
	...
	matrixA[0][0] = a;
	matrixA[0][1] = halving(b);
	matrixA[1][0] = halving(b);
	matrixA[1][1] = c;
	...
	cout << "x'^{T} = " << yt << endl; 
	cout << "D = " << MatrixD << endl; 
	cout << "x'^{T} * D * x' = " << yt*MatrixD*y << " = " << k << endl; 
	...
	{
		cout <<"\nMatrix A is positive definite, the conic is an ellipse" <<endl;
	}
	else if (real(eigval[0]) > 0 && real(eigval[1]) < 0 )
	{
		cout <<"\nMatrix A is indefinite, the conic is a hyperbola" <<endl;
	}
	else if (real(eigval[0]) < 0 && real(eigval[1]) < 0 )
	{
		cout <<"\nMatrix A is negative definite, the conic has no graph" <<endl;
	}
