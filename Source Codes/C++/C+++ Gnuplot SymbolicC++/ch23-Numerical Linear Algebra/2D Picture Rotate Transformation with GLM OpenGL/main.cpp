...
int main()
{
        ...
        while (window.pollEvent(windowEvent))
	{
        ...
        transrotate = glm::rotate(transrotate,
					glm::radians(rotatedirection), // to rotate continuously -> time*glm::radians(rotatedirection)
					glm::vec3(0.0f, 0.0f, 1.0f)); // rotate 90 degrees toward positive z axis 

	// glm::mat4 rMat = glm::mat4(1.0f);
	// rMat = glm::rotate(rMat, 1.75f*(float)time, glm::vec3(1.0f, 0.0f, 0.0f)); // rotate toward x-axis continuously
	// rMat = glm::rotate(rMat, 1.75f*(float)time, glm::vec3(0.0f, 1.0f, 0.0f)); // rotate toward y-axis continuously
	
        glUniformMatrix4fv(uniTrans, 1, GL_FALSE, glm::value_ptr(transrotate));
