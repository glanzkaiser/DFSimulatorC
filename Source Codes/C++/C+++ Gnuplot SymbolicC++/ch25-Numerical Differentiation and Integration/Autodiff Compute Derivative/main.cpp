...
int main()
{
	...
	auto const x{::boost::math::differentiation::make_fvar<double, 5>(variable)}; // to find derivative till order 5
	auto const dx{get_x_coordinate(x*x*x)};
	cout << "x = " << var_x <<endl;
	cout << "x in autodiff = " << x << endl;
	cout << "dx in autodiff = " << dx << endl;
	cout << "f(x) = x*x*x = " << dx.derivative(0) << endl; // sin(x) / sin(theta*DEGTORAD)
	cout << "d f(x) = 3*x*x = " << dx.derivative(1) << endl; // d sin(x)/d x = cos(x)
	cout << "d(d f(x))  = 6*x = " << dx.derivative(2) << endl; // -sin(x)
	cout << "d^2(d f(x)) = " << dx.derivative(3) << endl; // - cos(x)
	cout << "d^3(d f(x)) = " << dx.derivative(4) << endl; // sin(x)
	cout << "d^4(d f(x)) = " << dx.derivative(5) << endl; // cos(x)
	return 0;
}